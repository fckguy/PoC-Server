...
// const baseURL = "https://dev-testnet.wallaby.cash/api/v1";
...
const multisigInit = async () => {
  try {
    ...
    const signature = EthCrypto.sign(participants[0].credentials.clientAuthPrivKey, hashedMessage);

    const recipientAddress = participants[2].credentials.address;

    const initialSignerEncryptedSeed = await EthCrypto.encryptWithPublicKey(
      participants[0].credentials.wallabyAuthPubKey,
      participants[0].credentials.seedphrase,
    );

    const multiSignAssetOriginAddress = 'CBYK5WDHDKIARJZFZYHW7EDQPTDLYXY33BIGRZJCAXBLB7M26BSEZOL7AM';

    const payload = {
      multisigAssetId: 'b2ce718d-34a0-406a-bb3d-d4b7a1d33c8a',
      multiSignAssetOriginAddress,
      recipientAddress: recipientAddress,
      amount: 0.1,
      reference: v4(),
      memo: '',
      initialSignerEncryptedSeed,
    };
    ...
    const {
      ...
      `/transactions/init-multisig-transfer`,
      {
        clientAuthPubKey: participants[0].credentials.clientAuthPubKey,
        signature,
        message,

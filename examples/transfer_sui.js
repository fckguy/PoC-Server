...
const { RawSigner } = require('@mysten/sui/utils');
...
const testSimpleTransferTx = async () => {
  try {
    ...
    // 2. Sign the message
    const keypair = Ed25519Keypair.fromSecretKey(senderCredentials.clientAuthPrivKey);
    const signer = new RawSigner(keypair, new SuiClient());
